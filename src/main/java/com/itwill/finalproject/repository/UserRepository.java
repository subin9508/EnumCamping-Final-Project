...
public interface UserRepository extends JpaRepository<User, Integer> {
	...
	Integer checkUserIsActive(@Param("userId") String userId);
	 ...
	 // 로그인 시 비활성화 기간 확인
	@Query(value = "SELECT COUNT(*) FROM users WHERE user_key = :userKey AND deactive_until > CURRENT_DATE", nativeQuery = true)
	Integer checkDeactivationPeriod(@Param("userKey") String userKey);
