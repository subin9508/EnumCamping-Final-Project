...
/* document.addEventListener('DOMContentLoaded',()=>{
 ...
 document.addEventListener('DOMContentLoaded', () => {
     ...
     const inputTitle = document.querySelector('input#title');
     const textContent = document.querySelector('textarea#content');
     const qnaLockCheckbox = document.querySelector('input#qnaLock');
     ...
     btnUpdate.addEventListener('click', () => {
         ...
         const result = confirm('변경 내용을 저장할까요?');
         if (result) {
             // 체크박스의 상태를 확인하여 hidden input에 반영
             const qnaLockInput = document.createElement('input');
             qnaLockInput.type = 'hidden';
             qnaLockInput.name = 'qnaLock';
             qnaLockInput.value = qnaLockCheckbox.checked ? '1' : '0';
             modifyForm.appendChild(qnaLockInput);
